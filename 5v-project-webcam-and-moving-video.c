...
IplImage* findWhitespace(IplImage *reference, IplImage *template)
{
	...
	/* resultult image of template match*/
	...
	if (minval < THRESHOLD_VAL) {
		...
		cvRectangle( reference,cvPoint( minloc.x, minloc.y ), cvPoint( minloc.x + template_width, minloc.y + template_height ), cvScalar( 0, 0, 255, 0 ), 1, 0, 0 );	//draw square
		...
		return reference; //I can't find a way around the warning caused by this
	}
...
IplImage* processFrame(IplImage* frame)
{
	...
	//set to greyscale
	greyframe = cvCreateImage(cvGetSize(frame),IPL_DEPTH_8U,1);
	...
	//set template white
	cvRectangle(tpl, cvPoint(0,0),  cvPoint(frame->width, frame->height), cvScalarAll(255),CV_FILLED, 0, 0); 
...
{	
	...
	//for image process call see end
